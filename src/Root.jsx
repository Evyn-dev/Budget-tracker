...
export default function Root() {
  ...
  const [linkError, setLinkError] = useState(() => {
    ...
    return (hash.get('error_code') || query.get('error_code')) === 'otp_expired'
      ? 'This email link is invalid, has expired, or has already been used. Your password has not been changed. Request a new link using Forgot Password and open it once.'
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = client.auth.onAuthStateChange((event, session) => {
      ...
      setAuth(prev => ({ client, session, loading: false, recovering: event === 'PASSWORD_RECOVERY' || (event !== 'SIGNED_OUT' && prev.recovering) }));
    });
    client.auth.getSession().then(({ data, error }) => {
      if (active && !receivedAuthEvent) setAuth(prev => ({ ...prev, client, loading: false, session: error ? null : data.session }));
  ...
  if (auth.recovering) return <AuthScreen client={auth.client} recovering onRecovered={() => setAuth(prev => ({ ...prev, recovering: false }))} />;
